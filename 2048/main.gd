...
@onready var score_scene = preload("res://scores.tscn")
var food_array : Array
var array_range = 1000
var enemy_array : Array
var enemy_array_range = 50
...
func _ready():
	...
	for i in range(array_range):
		...
		food.position = Vector3(randi_range(-240, 240),0, randi_range(-240, 240))
	...
	for i in range(enemy_array_range):
		...
		var food_spawn_location = get_node("Path3D/PathFollow3D")
		#food_spawn_location.progress_ratio = randf()
		enemy.position = Vector3(randi_range(-240, 240),0, randi_range(-240, 240))
